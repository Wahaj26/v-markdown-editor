<template>
  <div
    :class="['v-md-container', css,{'v-md-auto-resize': height=='auto', 'v-md-fullscreen': fullScreen }]"
  >
    <div class="v-md-toolbar" v-if="toolbars.length>0">
      <div class="btn-group mr-3" role="group" v-for="group in toolbars">
        <button
          ...
          :title="i.title"
          :class="'btn btn-'+theme"
          v-on:click="command(i.cmd)"
          :disabled="preview && !i.ready"
          v-for="i in group"
        >
          <i :class="i.ico"></i>
        </button>
  ...
  methods: {
    ...
    build() {
      if (document.getElementById(this.id)) {
        ...
        var o = Object.assign(
          { mode: "markdown", extraKeys: shortcuts },
          this.options
        );
        var ed = (this.editor = CodeMirror.fromTextArea(
